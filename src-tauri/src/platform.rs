...
impl ProfilePersistence for FileProfilePersistence {
    ...
    fn save(&self, document: &[u8]) -> Result<(), String> {
        ...
        let temporary_file = temporary_file_for(&self.profile_file);
        let mut output = OpenOptions::new()
            .create(true)
            .truncate(true)
            .write(true)
            .open(&temporary_file)
            .map_err(|error| format!("Could not prepare the local profile: {error}"))?;
        output
            .write_all(document)
            .and_then(|_| output.sync_all())
            .map_err(|error| format!("Could not write the local profile: {error}"))?;
        fs::rename(&temporary_file, &self.profile_file)
            .map_err(|error| format!("Could not activate the local profile: {error}"))
    }
}
