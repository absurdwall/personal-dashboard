...
Verification completed: `npm run check`; `cargo test --manifest-path src-tauri/Cargo.toml` (73 passed); Rust format check; `bash -n scripts/acceptance/macos-ipc-workflow.sh`; Swift driver compilation; one fresh `npm run build:mac`; direct packaged desktop/compact inspection; and the final focused packaged list-first acceptance. No picker/time-selection workflow, `exceptions`, or recursive `gate` run was executed.
