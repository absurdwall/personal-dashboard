...
activity、duration、effort。`Under 20` 保留为 recorded-short，不增加 qualifying
progress；其他时长只完成一次 qualifying progress。当前 ticket 不需要重复修改
fixture 或生产 Rust seam。

## Verification

- Playwright fixture：选择 `Other exercise` → `Under 20` → `Very hard`，逐阶段进入下一步；最终 review 显示这三个实际选择，保存后 Monday 显示 short effort 且进度保持 `0 of 3`。
